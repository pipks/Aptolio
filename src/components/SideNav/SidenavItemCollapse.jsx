...
const SidenavItemCollapse = ({ icon, name, path, pathname, collapse }) => {
  ...
  return (
    <div className='py-1'>
      <div
        className={`${active ? 'bg-primary shadow-lg' : ''} group flex items-center justify-between space-x-2 cursor-pointer duration-150 hover:bg-darkCard w-full h-[50px] px-2 rounded-md`}
        onClick={() => (collapsed === name ? setIsCollapsed(!collapsed) : setIsCollapsed(!collapsed))}>
        <div className='flex items-center space-x-2'>
          <div className={`${active ? 'text-white' : 'text-darkText'} duration-150 group-hover:text-white text-xl`}>{icon}</div>
