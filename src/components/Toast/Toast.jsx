        ...
        <div className='flex-1 w-0 p-4'>
          <div className='flex items-start'>
            ...
            <div className='ml-3 flex-1'>
              <p className='text-sm font-medium text-darkText'>{title}</p>
              {message && <p className='mt-1 text-sm text-gray-500 '>{message}</p>}
              {txHash && (
                <a href={`${getExplorerURL('txn', txHash)}`} target='_blank' rel='noreferrer' className='mt-1 text-sm text-gray-500'>
                  View: {shortAddress(txHash, 6)}
                </a>
              )}
            </div>
          </div>
