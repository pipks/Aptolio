...
export const checkJungle = async (walletAddress) => {
  ...
  if (getData.status === 200) {
    if (getData.data.data.hasOwnProperty('adventure_ids')) {
      ...
      await Promise.all(monkeysList[0].map(async (x) => {
        const options = {
          method: 'POST',
    ...
    return { status: getData.data.error_code === 'resource_not_found' ? 200 : 404, statusCode: getData.data.error_code === 'resource_not_found' ? 'info' : 'error', statusText: getData.data.error_code === 'resource_not_found' ? 'User has no staked Aptos Monkeys' : getData.data.error_code, data: stakedMonkeys }
  }
